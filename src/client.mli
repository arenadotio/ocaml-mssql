...
val execute :
  ?params:Db_field.t option list -> t -> string -> Row.t list Deferred.t
...
val execute_pipe :
  ?params:Db_field.t option list -> t -> string -> Row.t Pipe.Reader.t

val execute_unit :
  ?params:Db_field.t option list -> t -> string -> unit Deferred.t
(** Like [execute] but asserts that the result set will be empty. *)

val execute_single :
  ?params:Db_field.t option list -> t -> string -> Row.t option Deferred.t
(** Like [execute] but asserts that the result set will return a
    maximum of 1 row. *)

val execute_multi_result :
  ?params:Db_field.t option list -> t -> string -> Row.t list list Deferred.t
...
val with_transaction_or_error :
  t -> (t -> 'a Deferred.Or_error.t) -> 'a Deferred.Or_error.t
